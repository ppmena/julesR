...
test_that("jules_session_create returns a session list", {
  ...
  testthat::with_mocked_bindings({
    suppressMessages(
      suppressWarnings(
        res <- jules_session_create(prompt = "test", source = "1")
      )
    )
...
test_that("jules_message_send works", {
  ...
  testthat::with_mocked_bindings({
    suppressMessages(suppressWarnings(res <- jules_message_send("1", "hi")))
...
test_that("jules_plan_approve works", {
  ...
  testthat::with_mocked_bindings({
    suppressMessages(suppressWarnings(res <- jules_plan_approve("1")))
    expect_true(res$done)
  }, jules_perform = function(...) mock_resp)
})
